...
class ResultsDisplayWindow2:
    ...
    def update_detail_view(self, *args):
        ...
        smr_frame = ctk.CTkFrame(self.detail_container)
        smr_frame.pack(side="left", fill="both", expand=True, padx=5, pady=5)
        
        if 'SMR Project' in selected_combo:
            smr_data = self.data_srm.get(selected_combo['SMR Project'], {})
            for key, value in smr_data.items():
        ...
        combo_frame = ctk.CTkFrame(self.detail_container)
        ...
        elec_frame = ctk.CTkFrame(self.detail_container)
        ...
        ctk.CTkLabel(smr_frame, text="SMR Details", 
                    font=("Helvetica", 16, "bold")).pack(pady=10)
        smr_data = self.data_srm[selected_combo['SMR Project']]
        ...
        if 'Electrolysis Technology' in selected_combo:
            elec_data = self.data_elec.get(selected_combo['Electrolysis Technology'], {})
        ...
        # Display electrolysis details
        ctk.CTkLabel(elec_frame, text="Electrolysis Details", 
                    font=("Helvetica", 16, "bold")).pack(pady=10)
        elec_data = self.data_elec[selected_combo['Electrolysis Technology']]
        for key, value in elec_data.items():
            ctk.CTkLabel(
                elec_frame,
                text=f"{key}: {value}"
